...
import dna.depr.metrics.similarityMeasures.jaccard.JaccardUndirectedIntWeightedU;
 ...
 */
public class MatchingUndirectedIntWeightedU extends
		MatchingUndirectedIntWeighted {

	/**
	 * Initializes {@link JaccardUndirectedIntWeightedU}.
	 */
	public MatchingUndirectedIntWeightedU() {
		super("MatchingUndirectedIntWeightedU",
	...
	private boolean applyBeforeEdgeWeightUpdate(
		...
		applyBeforeEdgeRemoval(undirectedIntWeightedEdge);
		undirectedIntWeightedEdge.setWeight(new IntWeight(weight));
		applyAfterEdgeAddition(undirectedIntWeightedEdge);
		return true;
	}
	 ...
	 */
	private void decreaseMatching(HashMap<UndirectedNode, Integer> map,
			UndirectedNode node) {
	 ...
	 */
	private void decreaseMatching(UndirectedNode node1, int value1,
			UndirectedNode node2, int value2) {
	...
	private void decreaseMatchingNodeRemove(UndirectedNode nodeToRemove) {
		for (Entry<UndirectedNode, Integer> node1 : this.getNeighborNodes(
				nodeToRemove).entrySet())
			for (Entry<UndirectedNode, Integer> node2 : this.getNeighborNodes(
					nodeToRemove).entrySet()) {
	 ...
	 */
	private void increaseMatching(HashMap<UndirectedNode, Integer> map,
			UndirectedNode node) {
		for (Entry<UndirectedNode, Integer> node1 : map.entrySet())
	 ...
	 */
	private void increaseMatching(UndirectedNode node1, int value1,
			UndirectedNode node2, int value2) {
