...
public class IntWeightedAllPairsShortestPathsU extends
			...
			if (DirectedNode.class.isAssignableFrom(this.g
				...
				while (!q.isEmpty()) {
					...
					for (IElement iEdge : current.getOutgoingEdges()) {
						...
						int alt = height.get(current)
								+ (int) dWeight.getWeight();
						if (alt < 0) {
							continue;
						}
	...
	public boolean applyAfterUpdate(EdgeAddition ea) {
		if (this.g.isDirected()) {
			DirectedWeightedEdge e = (DirectedWeightedEdge) ea.getEdge();
			for (IElement ie : g.getNodes()) {
			...
			UndirectedWeightedEdge e = (UndirectedWeightedEdge) ea.getEdge();
			for (IElement ie : g.getNodes()) {
	...
	public boolean applyAfterUpdate(EdgeWeight ew) {
		if (this.g.isDirected()) {
			DirectedWeightedEdge e = (DirectedWeightedEdge) ew.getEdge();
			...
			for (IElement ie : g.getNodes()) {
				...
				if (height.get(src) + (int) eWeight.getWeight() == height
						.get(dst)) {
					continue;
				}
			...
			UndirectedWeightedEdge e = (UndirectedWeightedEdge) ew.getEdge();
			...
			for (IElement ie : g.getNodes()) {
				...
				if (!parent.containsKey(dst)
						|| height.get(src) + (int) eWeight.getWeight() == height
								.get(dst)) {
					continue;
				}
