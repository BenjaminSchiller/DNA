 ...
 */
public class JaccardUndirectedDoubleWeightedU extends
		JaccardUndirectedDoubleWeighted implements IBeforeUpdatesWeighted,
		IAfterUpdatesWeighted {

	/**
	 * Initializes {@link JaccardUndirectedDoubleWeightedU}.
	 */
	public JaccardUndirectedDoubleWeightedU() {
		super("JaccardUndirectedDoubleWeightedU");
	}
	...
	private void addNeighbor(UndirectedWeightedEdge newEdge) {
		if (this.neighborNodes.containsKey(newEdge.getNode1()))
			...
			HashMap<UndirectedNode, Double> map = new HashMap<UndirectedNode, Double>();
	...
	private boolean applyBeforeNodeRemoval(NodeRemoval u) {
		...
		HashMap<UndirectedNode, Double> neighborsNodeToRemove = this
	...
	private void removeFromNeighborNodes(UndirectedNode nodeToRemove) {
		this.neighborNodes.remove(nodeToRemove);
		for (Entry<Node, HashMap<UndirectedNode, Double>> iterable_element : this.neighborNodes
				.entrySet())
	...
	protected void update(UndirectedNode node1, UndirectedNode node2) {
		HashMap<UndirectedNode, Double> denominator = this.getUnion(
