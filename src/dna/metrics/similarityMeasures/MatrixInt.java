...
public class MatrixInt {
	...
	// <ROW, <COLUMN, value>
	private Map<Node, Map<Node, Integer>> values;

	/**
	 * Creates an empty {@link MatrixInt}.
	 */
	public MatrixInt() {
		this.values = new HashMap<Node, Map<Node, Integer>>();
	}

	public boolean equals(MatrixInt m, double faultTolerance) {
		boolean equals;
	...
	public boolean equals(Object o) {
		if (o != null && o instanceof MatrixInt)
			return ((MatrixInt) o).values.equals(this.values);
	 ...
	 *         exist.
	 */
	public Integer get(Node row, Node column) {
	 ...
	 *            The value to insert.
	 */
	public void put(Node row, Node column, Integer value) {
		if (row.getIndex() < column.getIndex()) {
			Node tmp = row;
			row = column;
			column = tmp;
		...
		if (this.values.containsKey(row))
			...
			final Map<Node, Integer> newRow = new HashMap<Node, Integer>();
	...
	/**
	 * Returns a string representation of this {@link MatrixInt} for all current
	 ...
	 * 
	 * 
	 * @return The entries of this {@link MatrixInt}.
	 */
	@Override
